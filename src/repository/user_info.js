...
class User_infoRepository {


  async create(userInf){
    return await user_info.create(userInf)
  }

  async update(id, userData){
    return await user_info.update (userData,{where: {id}});
  }

  async delete(id){
    return await user_info.destroy({where:{id}});
  }
  ...
  async findId(id){
    return await user_info.findOne({where:{id}});
  }
...
module.exports = new User_infoRepository();
