...
class FeedbackService {
  ...
  async createFeedback(feedbackInf){
    return await feedback.create(feedbackInf)
  }

  async updateFeedback(id, feedbackData){
    const res =await feedback.update(id,feedbackData);
    if (res){
      return res;
    }
    else{
  ...
  async deleteFeedback(id){
    const res =await feedback.delete(id);
    if (res){
      return res;
    }
    else{
  ...
  async findFeedbackById(id){
    const res =await feedback.findId(id);
    if (res){
      return res;
    }
    else{
