const speciality_test = require("../repository/speciality_test");
const NotAcceptableError = require("../errors/NotAcceptableError");


class speciality_testService {


  async createSpeciality_test(speciality_testInf){
    return await speciality_test.create(speciality_testInf)
  }

  async updateSpeciality_test(id, speciality_testData){
    const res =await speciality_test.update(id,speciality_testData);
    if (res){
      return res;
    }
    else{
  ...
  async deleteSpeciality_test(id){
    const res =await speciality_test.delete(id);
    if (res){
      return res;
    }
    else{
  ...
  async listSpeciality_test(){
    return await speciality_test.findAll();

  }

  async findSpeciality_testById(id){
    const res =await speciality_test.findId(id);
    if (res){
      return res;
    }
    else{
...
module.exports = new speciality_testService();
