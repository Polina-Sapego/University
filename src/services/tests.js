const tests = require("../repository/tests");
...
class testsService {
  ...
  async createTests(testsInf){
    return await tests.create(testsInf)
  }

  async updateTests(id, testsData){
    const res =await tests.update(id,testsData);
    if (res){
      return res;
    }
    else{
  ...
  async deleteTests(id){
    const res =await tests.delete(id);
    if (res){
      return res;
    }
    else{
  ...
  async findTestsById(id){
    const res =await tests.findId(id);
    if (res){
      return res;
    }
    else{
